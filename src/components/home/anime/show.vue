        ...
        <div class="main-left">
          ...
          <div class="file">
            ...
            <div class="file-info clearfix">
              ...
              <template v-for="(item,index) in fileData">
                <a :href="'/#/anime/detail/' + data.id + '?file=' + index" class="file float-left">{{index + 1}}.{{item.file_name}}</a>
              </template>
          ...
          <div class="commit">
            ...
            <template v-if="commitData.length" v-for="(item,index) in commitData">
              <commit :imgSize="'58px'" :isShowAnime="false"
...
<script>
  import {getShowAnimeList,getAnimeList} from 'api/anime'
  ...
  export default {
    ...
    methods:{
      _favChange() {
        postStoreFav(this.data.id,'anime').then(res => {
      ...
      _hasFav(){
        gethasFav(this.userNow.id,'anime', this.$route.params.id).then(res => {
          if (res.meta.errno === has_delete){
            this.isFav = true
          }
          if (res.meta.errno === has_store){
      ...
      _commitSubmit(commit) {
        postStoreCommit(this.data.id,'anime',commit).then(res => {
          if (res.meta.errno === ERR_OK){
