...
<script>
  import {getPictureList,deletePicture} from 'api/picture';
  ...
  export default {
    ...
    methods: {
      ...
      show (index) {
        this.$router.push({ path: '/picture/' + this.data[index].id, })
      },
      update (index) {
        this.$router.push({ path: '/admin/picture/' + this.data[index].id, })
      },
      openDelModel (index) {
        this.delModel = true;
        this.readyDel = index;
      ...
      del() {
        deletePicture(this.data[this.readyDel].id).then(res => {
          if (res.meta.errno === ERR_OK){
