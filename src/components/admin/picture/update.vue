<template>
  <div class="kkz-admin-picture-create">
...
<script>
  ...
  import {postUpdatePictureList,getTopicList,getShowPictureList} from 'api/picture';
  import {ERR_OK} from 'api/config';
  import {postImageDelete} from 'api/image'
  import Multiselect from 'vue-multiselect'
  export default {
    components: {ICol,Multiselect},
    data () {
      return {
        imgData: {
          width: 300,
    ...
    methods: {
      ...
      handleSuccess (res, file) {
        if (res.meta.errno === ERR_OK){
          this.formValidate.thumb = res.data.small;
          this.formValidate.big_thumb = res.data.big;
      ...
      handleSubmit(name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            postUpdatePictureList(this.formValidate).then(res => {
              if (res.meta.errno === ERR_OK){
